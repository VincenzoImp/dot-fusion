...
const MySwaps: NextPage = () => {
    ...
    // Read swap events from the contract
    const { data: swapCreatedEvents } = useScaffoldEventHistory({
    ...
    const { data: swapCompletedEvents } = useScaffoldEventHistory({
        contractName: "DotFusionEthereumEscrow",
    ...
    const { data: swapCancelledEvents } = useScaffoldEventHistory({
        contractName: "DotFusionEthereumEscrow",
    ...
    // Process events and create swap data
    useEffect(() => {
        if (!swapCreatedEvents || !connectedAddress) {
            setLoading(false);
            return;
        }
        ...
        swapCreatedEvents.forEach(event => {
            const swapData = event.args;
            if (swapData && (swapData.maker === connectedAddress || swapData.taker === connectedAddress)) {
                // Check if swap was completed
                const isCompleted = swapCompletedEvents?.some(
                    completedEvent => completedEvent.args?.swapId === swapData.swapId,
                );

                // Check if swap was cancelled
                const isCancelled = swapCancelledEvents?.some(
                    cancelledEvent => cancelledEvent.args?.swapId === swapData.swapId,
                );

                let state: SwapState = "OPEN";
                ...
                processedSwaps.push({
                    ...
                    taker: swapData.taker || "",
                    token: swapData.token || "",
                    amount: swapData.amount?.toString() || "0",
                    unlockTime: swapData.unlockTime?.toString() || "0",
                    state,
        ...
        processedSwaps.sort((a, b) => b.blockNumber - a.blockNumber);
        setSwaps(processedSwaps);
        setLoading(false);
    }, [swapCreatedEvents, swapCompletedEvents, swapCancelledEvents, connectedAddress]);
